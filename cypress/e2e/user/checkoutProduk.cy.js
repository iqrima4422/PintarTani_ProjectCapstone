...
describe('Profile Setting', () => {
    ...
    it('User Can Add Produk to Cart', () => {
        cy.visit("http://127.0.0.1:8000/login");
        cy.get(':nth-child(2) > .form-control').type("user@gmail.com");
        cy.get(':nth-child(3) > .form-control').type("12345678");
        cy.get('.btn').click();
        cy.get('[href="http://127.0.0.1:8000/gallery"]').click();
        cy.get('.display-3').should("have.text", "Produk Penjualan");
        cy.get('.breadcrumb').should("have.text", "Beranda / Produk");
    ...
    it('User Can Checkout a Product', () => {
        ...
        cy.visit("http://127.0.0.1:8000/purchase");
    })
})
